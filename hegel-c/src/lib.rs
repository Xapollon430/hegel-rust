...
/// Parameters:
/// `out`: Receives the configured stateful step count.
///
/// Returns `HEGEL_OK`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn hegel_settings_get_stateful_step_count(
    ctx: *mut HegelContext,
    s: *const HegelSettings,
    out: *mut i64,
) -> hegel_result_t {
    clear_last_error(ctx);
    let handle = match unsafe { settings_ref(ctx, s, "hegel_settings_get_stateful_step_count") } {
        Ok(h) => h,
        Err(rc) => return rc,
    };
    if out.is_null() {
        set_last_error(
            ctx,
            "hegel_settings_get_stateful_step_count: out parameter is null",
        );
        return HEGEL_E_INVALID_ARG;
    }
    unsafe { *out = handle.inner.stateful_step_count };
    HEGEL_OK
}
