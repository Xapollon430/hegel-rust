...
pub(crate) struct ProfileDelta {
    ...
    pub(crate) stateful_step_count: Option<i64>,
...
impl ProfileDelta {
    ...
    pub(crate) fn apply(&self, settings: &mut Settings) {
        ...
        if let Some(v) = self.stateful_step_count {
            settings.stateful_step_count = v;
        }
    ...
    pub(crate) fn snapshot(settings: &Settings) -> Self {
        Self {
            ...
            stateful_step_count: Some(settings.stateful_step_count),
