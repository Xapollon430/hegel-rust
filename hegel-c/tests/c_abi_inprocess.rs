...
unsafe fn read_settings(ctx: *mut HegelContext, s: *const hegel_c::HegelSettings) -> SettingsView {
    unsafe {
        ...
        let mut steps = 0i64;
        ok(hegel_c::hegel_settings_get_stateful_step_count(
            ctx, s, &mut steps,
        ));
...
fn settings_getters_read_back_every_setter() {
    ...
    unsafe {
        ...
        ok(hegel_c::hegel_settings_set_stateful_step_count(ctx, s, 9));
        ok(hegel_c::hegel_settings_set_verbosity(
            ctx,
            s,
            hegel_c::hegel_verbosity_t::HEGEL_VERBOSITY_DEBUG as u32,
        ...
        assert_eq!(view.stateful_step_count, 9);
        assert_eq!(
            view.verbosity,
            hegel_c::hegel_verbosity_t::HEGEL_VERBOSITY_DEBUG as u32
        );
...
fn the_default_profile_reads_back_the_base_defaults() {
    ...
    unsafe {
        ...
        assert_eq!(view.stateful_step_count, 50);
        assert_eq!(
            view.verbosity,
            hegel_c::hegel_verbosity_t::HEGEL_VERBOSITY_NORMAL as u32
        );
...
fn settings_getters_reject_null_handles_and_null_outs() {
    ...
    unsafe {
        ...
        let mut i = 0i64;
        assert_eq!(
            hegel_c::hegel_settings_get_stateful_step_count(ctx, null_s, &mut i),
            HEGEL_E_INVALID_HANDLE
        );
        assert_eq!(
            hegel_c::hegel_settings_get_stateful_step_count(ctx, s, ptr::null_mut()),
            HEGEL_E_INVALID_ARG
        );
